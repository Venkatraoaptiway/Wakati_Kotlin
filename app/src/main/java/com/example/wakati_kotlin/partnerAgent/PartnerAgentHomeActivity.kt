package com.example.wakati_kotlin.partnerAgent
...
import com.example.wakati_kotlin.databinding.ActivityPartnerAgentHomeBinding
import com.example.wakati_kotlin.reciever.DealersTotalListActivity
...
class PartnerAgentHomeActivity() : AppCompatActivity() {
    var binding: ActivityPartnerAgentHomeBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityPartnerAgentHomeBinding.inflate(
            layoutInflater
        )
...
//        Log.d("USER_ID", userId);
        ...
        binding!!.profileImage.setOnClickListener(View.OnClickListener {
            val intent = Intent(this@PartnerAgentHomeActivity, ProfileActivity::class.java)
            intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT)
            startActivity(intent)
        })
        binding!!.logoutIcon.setOnClickListener {
            val intent = Intent(this@PartnerAgentHomeActivity, LoginActivity::class.java)
            startActivity(intent)
        }

        binding!!.transactions.setOnClickListener {
            val intent = Intent(this@PartnerAgentHomeActivity, TransactionsListActivity::class.java)
            startActivity(intent)
        }
        ...
        binding!!.superDealerLayout.setOnClickListener {
            ...
            intent.putExtra("flag","Total_PA_Super_Dealers_Flow");
            startActivity(intent)
    ...
    fun serviceProfile(authToken: String?, userId: String?) {
        ...
        retrofitClass.dashboard(object : Callback<LoginResponse> {
            ...
            override fun onFailure(call: Call<LoginResponse?>, t: Throwable) {
                binding!!.swipeRefresh.isRefreshing = false
                LoaderUtils.hideLoader()
                Toast.makeText(this@PartnerAgentHomeActivity, "API ERROR", Toast.LENGTH_SHORT)
                    .show()
            }
        }, (authToken)!!, (userId)!!)
    }
        ...
        retrofitClass.receiverPartnerAgentDashboard(object : Callback<DashboardResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    // Cash Summary
                    binding!!.availableBalance.text =
                        formatKmf(dashboard!!.cash_summary!!.current_balance.toString())
                    binding!!.walletBalance.text =
                        formatKmf(dashboard.cash_summary!!.wallet_balance.toString())
                    binding!!.paymentsMade.text =
                       formatKmf(dashboard.cash_summary!!.payments_collected.toString())
                    binding!!.paymentsCollected.text =
                        formatKmf(dashboard.cash_summary!!.payments_made.toString())
    ...
    fun serviceTransactions(authToken: String?, userId: String?) {
        ...
        retrofitClass.getTransactions(object : Callback<TransactionResponse> {
            ...
            override fun onFailure(call: Call<TransactionResponse?>, t: Throwable) {
                binding!!.swipeRefresh.isRefreshing = false
                Log.e("API_ERROR", (t.message)!!)
                Toast.makeText(this@PartnerAgentHomeActivity, t.message, Toast.LENGTH_LONG).show()
            }
        }, (authToken)!!, jsonObject)
    }
    ...
    private fun fetchProfileImage(documentId: String) {
        ...
        client.newCall(request).enqueue(object : okhttp3.Callback {
            override fun onFailure(call: okhttp3.Call, e: IOException) {
                runOnUiThread{
                    LoaderUtils.hideLoader()
                    Toast.makeText(this@PartnerAgentHomeActivity, e.message, Toast.LENGTH_SHORT)
                        .show()
                }
            }
